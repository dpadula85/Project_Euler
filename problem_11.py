# -*- coding: utf-8 -*-
"""
Created on Sat Mar 14 14:49:26 2015
...
#
#grid store in file problem_11.txt

import numpy as np
import time

path = 'C:\\Users\\Daniele\\Desktop\\python_practice\\project_Euler\\'
file_grid = 'problem_11.txt'
m = np.loadtxt(path + file_grid)
...
#line and column products
for i in range(w):
    for j in range(h - b):
        #product of line elements
        prod = m[i, j] * m[i, j + 1] * m[i, j + 2] * m[i, j + 3]
        prods.append(prod)
        #product of column elements
        # j, i inverted to avoid list index error when i = 17, 18, 19
        prod = m[j, i] * m[j + 1, i] * m[j + 2, i] * m[j + 3, i]
        prods.append(prod)
        
#right/down diagonal products
for i in range(w - b):
    for j in range(h - b):
        prod = m[i, j] * m[i + 1, j + 1] * m[i + 2, j + 2] * m[i + 3, j + 3]
        prods.append(prod)

#left/down diagonal products
...
print max(prods), 'in %5.3f seconds' % elapsed